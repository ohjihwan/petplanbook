      ...
      searchButton.addEventListener("click", function () {
        ...
        places.keywordSearch(keyword, function (results, status) {
          if (status === kakao.maps.services.Status.OK) {
            ...
            marker.setPosition(
              new kakao.maps.LatLng(results[0].y, results[0].x)
            );
          } else {
            alert("검색 결과가 없습니다.");
          }
...
function addToRoute(place) {
  ...
  // 현재 루트의 모든 장소 수집
  const currentPlaces = Array.from(
    routeList.querySelectorAll(".item:not(.-placeholder)")
  ).map((item) => {
  ...
  $(".make-route").sortable({
    ...
    update: function (event, ui) {
      ...
      const places = Array.from(
        this.querySelectorAll(".item:not(.-placeholder)")
      ).map((item) => {
        const name = item.querySelector(".name").textContent;
        const addr1 = item.querySelector(".info dd:nth-child(2)").textContent;
        const tel = item.querySelector(".info dd:nth-child(4)").textContent;
...
// 리스트의 장소들을 지도에 표시하는 함수
function displayListPlaces() {
  // 기존 마커와 경로선 제거
  if (routeMarkers.length > 0) {
    routeMarkers.forEach((marker) => marker.setMap(null));
    routeMarkers = [];
  ...
  // 리스트에서 장소 정보 수집
  const placeItems = document.querySelectorAll(
    ".following-path .swiper-slide li"
  );
  const places = [];

  placeItems.forEach((item) => {
    const name = item.querySelector("strong").textContent;
    const address = item.querySelector("dd").textContent;
    ...
    geocoder.addressSearch(address, function (result, status) {
      if (status === kakao.maps.services.Status.OK) {
        ...
        // 모든 장소가 처리되면 경로선 그리기
        if (places.length === placeItems.length) {
